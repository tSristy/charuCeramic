...
const MetaTagList = () => {
    ...
    useEffect(() => {
        ...
        ServerApi(`/meta/list`, 'POST', null, body)
            .then(res => res.json())
            .then(res => {
                setMetaList(res.items);
    ...
    const handleDelete = (id) => {
        const tempArr = metaList.filter(meta => meta.id !== id);
    ...
    const handlePanel = (arg)=>{
        ...
        else navigate(`/meta-panel?id=${arg}`);
    };
        ...
        <Box py={5}>
            <Container>
                <Stack direction={{ sm: "column", md: "row" }} justifyContent="space-between" alignItems="center">
                    <Box mb={3}>
                        <Typography variant="h5" fontWeight={600} mt={5} mb={1}>Records Explorer  <Chip label="Meta Tag" color="error" /></Typography>
                    ...
                    <Box sx={{ width: { sm: "100%", md: "30%" }, display: "flex", gap: 1 }}>
                        ...
                        />
                        <BtnAdminSubmit text="Create" onClick={(e)=>handlePanel('/meta-panel')} />
                    </Box>
                </Stack>
